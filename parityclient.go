// derived from ethclient
// ethclient Copyright 2016 The go-ethereum Authors
...
// Package parityclient provides a client for the Parity RPC API.
...
// Log - Tx Log
type Log struct {
	Address             math.HexOrDecimal256 `json:"address"`
	BlockHash           math.HexOrDecimal256 `json:"blockHash"`
...
// NewFilter returns a filter query.
func (ec *Client) NewFilter(ctx context.Context, q ethereum.FilterQuery) (*big.Int, error) {
	var result hexutil.Big
	err := ec.c.CallContext(ctx, &result, "eth_newFilter", toFilterArg(q))
	return (*big.Int)(&result), err
}
